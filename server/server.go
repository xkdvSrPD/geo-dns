...
func (c *Coordinator) resolveConcurrently(ctx context.Context, q *dns.Msg) (*dns.Msg, string, string) {
	...
	for received := 0; received < len(c.NS); received++ {
		select {
		case <-deadline:
			if groupFirstResp != nil {
				return groupFirstResp, chosenGroup, groupFirstNs
			}
		...
		case r := <-resCh:
			...
			if r.Msg == nil {
				log.Printf("level=error trace=%s event=upstream client_ip=%s ns=%s qname=%s duration=%s err=nil empty_msg", traceID, clientIP, r.NsName, qname, r.Duration)
				continue
			}
			...
			if chosenGroup == "" {
				...
				groupMembers = map[string]struct{}{}
				for _, name := range c.GroupMap[chosenGroup] {
					groupMembers[name] = struct{}{}
				}
			}
